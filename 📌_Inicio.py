...
if 'RIF' in st.session_state:
  ...
  info_cli = pd.read_sql(query, conn)
  col1, col2 = st.columns(2)
  ...
  try:
    cod_cli = cod_cliente(st.session_state['RIF'], conn)
    st.session_state['cod_cli'] = cod_cli
    # cuentas del cliente
    query = '''
    select
        g300uuid as key,
    ...
    query = query.format(txt = cod_cli)
    cuentas_cli = pd.read_sql(query, conn)
    
    # tabla de codigos producto
    cod_prod = pd.read_sql("select cod_producto, cuenta from cod_productos", conn)
    ...
    query = query.format(cod_cli = cod_cli)
    client_prod = pd.read_sql(query, conn)
    
    # tabla catalogo
    base_catalogo = pd.read_sql("select * from catalogo", conn)
